...
import { GET_USER_BY_SLUG, GET_TOURNAMENT_ENTRANTS, SEARCH_RECENT_TOURNAMENTS } from '@/lib/queries';
...
export default function PlayerSearch({ onPlayerSelect }: PlayerSearchProps) {
  ...
  const searchBySlug = async () => {
    ...
    if (data?.user) {
      ...
      setError('Player not found. Try searching in recent tournaments instead.');
    }
  };

  const searchByTournament = async () => {
    // First get recent tournaments
    const { data: tournamentsData } = await getRecentTournaments({ variables: { perPage: 10 } });
    ...
    for (const tournament of tournamentsData.tournaments.nodes.slice(0, 3)) {
      try {
        ...
        if (participantsData?.tournament?.participants?.nodes) {
          const tournamentPlayers = participantsData.tournament.participants.nodes.map((participant: { id: string; gamerTag: string; prefix?: string; user?: User }) => ({
            id: participant.id,
        ...
        console.warn(`Failed to search tournament ${tournament.slug}:`, err);
      }
    }

    if (results.length === 0) {
      setError('No players found in recent tournaments. Try a different search term.');
    } else {
        ...
        <div className="mb-4">
          <div className="flex gap-4 justify-center">
            <label className="flex items-center gap-2">
              ...
              />
              <span className="text-sm text-gray-700 dark:text-gray-300">By User ID</span>
            </label>
            <label className="flex items-center gap-2">
              <input
                type="radio"
                ...
                className="text-blue-600"
              />
              <span className="text-sm text-gray-700 dark:text-gray-300">In Tournaments</span>
            </label>
          </div>
        </div>
            ...
            >
              ...
              <div className="flex-1">
                <div className="font-semibold text-gray-900 dark:text-gray-100">
                  {player.prefix && <span className="text-gray-600 dark:text-gray-400">{player.prefix} | </span>}
                  {player.gamerTag}
                </div>
